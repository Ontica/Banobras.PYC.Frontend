...
export class PaymentOrderHeaderComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private setFormData() {
    setTimeout(() => {
      this.form.reset({
        organizationalUnitUID: FormHelper.getUIDValueValid(this.paymentOrder.requestedBy),
        paymentOrderTypeUID: FormHelper.getUIDValueValid(this.paymentOrder.paymentOrderType),
